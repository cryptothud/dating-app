...
export function MessagesWaitingEmail({ senderName, conversationUrl }: Props) {
  return (
    <Layout preview={`${senderName} sent you a message on CRUSH`}>
      ...
      <Text style={body}>
        <strong style={{ color: TEXT }}>{senderName}</strong> sent you a message on CRUSH. Jump back in and keep the conversation going.
      </Text>
      <Button href={conversationUrl} style={btn}>Read message</Button>
      <Text style={hint}>
        This is a one-time notification. We won&apos;t send another for this conversation until you&apos;ve had a chance to reply.
      </Text>
    </Layout>
  )
}
