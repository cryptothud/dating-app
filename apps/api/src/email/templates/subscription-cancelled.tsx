...
export function SubscriptionCancelledEmail({ accessUntil, reactivateUrl }: Props) {
  return (
    <Layout preview="Your CRUSH Premium subscription has been cancelled">
      ...
      <Text style={body}>
        Your CRUSH Premium subscription has been cancelled. You&apos;ll keep full access until{' '}
        <strong style={{ color: TEXT }}>{accessUntil}</strong>, then your account will revert to the
        free tier.
      </Text>
      <Text style={body}>
        Change your mind? You can reactivate any time — no waiting period, no setup fee.
      </Text>
      <Button href={reactivateUrl} style={btn}>
        Reactivate Premium
      </Button>
    </Layout>
  )
