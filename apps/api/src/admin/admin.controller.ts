...
export class AdminController {
  ...
  async warn(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: ReasonDto) {
    await this.admin.warnUser(req.user.id, id, dto.reason)
  }

  @Post('users/:id/suspend')
  @HttpCode(HttpStatus.NO_CONTENT)
  async suspend(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: ReasonDto) {
    await this.admin.suspendUser(req.user.id, id, dto.reason)
  }

  @Post('users/:id/unsuspend')
  @HttpCode(HttpStatus.NO_CONTENT)
  async unsuspend(@Req() req: AdminRequest, @Param('id') id: string) {
    await this.admin.unsuspendUser(req.user.id, id)
  }

  @Post('users/:id/ban')
  @HttpCode(HttpStatus.NO_CONTENT)
  async ban(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: ReasonDto) {
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async verify(@Req() req: AdminRequest, @Param('id') id: string) {
    await this.admin.verifyUser(req.user.id, id)
  }

  @Post('users/:id/force-logout')
  @HttpCode(HttpStatus.NO_CONTENT)
  async forceLogout(@Req() req: AdminRequest, @Param('id') id: string) {
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async timeout(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: TimeoutDto) {
  ...
  @UseGuards(ModeratorGuard)
  @HttpCode(HttpStatus.NO_CONTENT)
  async clearTimeout(@Req() req: AdminRequest, @Param('id') id: string) {
    await this.admin.clearTimeout(req.user.id, id)
  }

  @Post('users/:id/set-role')
  @HttpCode(HttpStatus.NO_CONTENT)
  async setRole(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: SetRoleDto) {
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async resolveReport(@Req() req: AdminRequest, @Param('id') id: string, @Body() dto: ReasonDto) {
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async setMaintenance(@Req() req: AdminRequest, @Body() dto: MaintenanceModeDto) {
    await this.admin.setMaintenanceMode(req.user.id, dto.enabled)
  }

  @Post('system/flush-cache')
  @HttpCode(HttpStatus.NO_CONTENT)
  async flushCache(@Req() req: AdminRequest, @Body() dto: FlushCacheDto) {
    await this.admin.flushCache(req.user.id, dto.pattern)
  }

  @Post('system/broadcast-push')
  @HttpCode(HttpStatus.NO_CONTENT)
  async broadcastPush(@Req() req: AdminRequest, @Body() dto: BroadcastPushDto) {
