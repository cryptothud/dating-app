...
export function BlockReportSheet({ userId, displayName, onClose, onBlocked }: Props) {
  ...
  const canActOnTarget =
    targetStatus !== null &&
    (isAdmin ? targetStatus.role !== 'admin' : targetStatus.role === 'user')

  const isTimedOut =
    !!targetStatus?.timeoutUntil && new Date(targetStatus.timeoutUntil) > new Date()
  ...
  const modActionLabel =
    pendingAction === 'warn' ? 'Warn' : pendingAction === 'suspend' ? 'Suspend' : 'Ban'
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="bg-background border-border absolute bottom-0 left-0 right-0 z-[90] rounded-t-2xl border-t px-4 pb-8 pt-5 shadow-2xl"
        ...
        >
          ...
          <div className="relative mb-5 flex justify-center">
            ...
            <button
              onClick={onClose}
              aria-label="Close"
              className="bg-muted/60 text-muted-foreground hover:text-foreground hover:bg-muted absolute right-0 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full transition-colors"
            >
              <svg
                viewBox="0 0 14 14"
          ...
          {step === 'menu' && (
            <div className="space-y-2">
              <p className="text-foreground mb-4 text-sm font-semibold">{displayName}</p>

              <button
                onClick={() => setStep('report-reason')}
                className="hover:bg-muted flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors"
              >
                <span className="text-amber-500">
                  <FlagIcon />
                </span>
                <span className="text-foreground text-sm font-medium">Report</span>
              </button>

              <button
                onClick={() => void block()}
                disabled={loading}
                className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors hover:bg-red-500/10 disabled:opacity-50"
              >
              ...
              {isMod && canActOnTarget && (
                <>
                  ...
                  <p className="text-muted-foreground mb-2 px-1 text-[10px] font-semibold uppercase tracking-widest">
                    {isAdmin ? 'Admin' : 'Mod'} Actions
                  </p>

                  <button
                    onClick={() => openModAction('warn')}
                    className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors hover:bg-amber-500/10"
                  >
                    <span className="text-amber-500">
                      <WarnIcon />
                    </span>
                    <span className="text-foreground text-sm font-medium">Warn</span>
                  </button>
                  ...
                  {targetStatus?.suspended ? (
                    <button
                      ...
                      disabled={loading}
                      className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors hover:bg-sky-500/10 disabled:opacity-50"
                    >
                      <span className="text-sky-500">
                        <UnsuspendIcon />
                      </span>
                      <span className="text-foreground text-sm font-medium">Unsuspend</span>
                    </button>
                  ) : (
                    <button
                      onClick={() => openModAction('suspend')}
                      className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors hover:bg-orange-500/10"
                    >
                      <span className="text-orange-500">
                        <SuspendIcon />
                      </span>
                      <span className="text-foreground text-sm font-medium">Suspend</span>
                    </button>
                  )}
                  ...
                  {targetStatus?.banned ? (
                    ...
                    >
                      ...
                      <span className="text-foreground text-sm font-medium">Unban</span>
                    </button>
                  ) : (
                    <button
                  ...
                  {isTimedOut && (
                    ...
                    >
                      ...
                      <span className="text-foreground text-sm font-medium">Remove Timeout</span>
                    </button>
                  )}
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="hover:bg-muted flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors disabled:opacity-50"
                  >
                    <span className="text-muted-foreground">
                      <ForceLogoutIcon />
                    </span>
                    <span className="text-foreground text-sm font-medium">Force Logout</span>
                  </button>
                </>
              )}
          ...
          {step === 'mod-reason' && (
            <div className="space-y-4">
              ...
              <textarea
                ...
                rows={3}
                maxLength={500}
                className="border-input bg-muted/30 text-foreground placeholder:text-muted-foreground focus:ring-primary/40 w-full resize-none rounded-xl border px-3 py-2.5 text-sm focus:outline-none focus:ring-1"
              />
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="border-border text-muted-foreground hover:text-foreground hover:bg-muted/50 flex-1 rounded-xl border py-3 text-sm font-medium transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className={[
                    'flex-1 rounded-xl py-3 text-sm font-semibold transition-opacity',
                    pendingAction === 'ban'
          ...
          {step === 'report-reason' && (
            <div className="space-y-3">
              <h2 className="text-foreground mb-1 text-sm font-semibold">What&apos;s the issue?</h2>
              {REPORT_REASONS.map((r) => (
                <button
                  ...
                  className="border-border hover:border-primary/50 hover:bg-muted/50 flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition-colors"
                >
          ...
          {step === 'report-details' && (
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={loading}
                className="bg-primary w-full rounded-xl py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-50"
              >
          ...
          {step === 'done' && (
            <div className="space-y-3 py-4 text-center">
              ...
              <p className="text-foreground text-sm font-semibold">Report submitted</p>
              <p className="text-muted-foreground text-xs">
                Thanks for keeping CRUSH safe. Our team will review this.
              </p>
