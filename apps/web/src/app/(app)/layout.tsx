...
export default function AppLayout({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    probe.style.cssText =
      'position:fixed;opacity:0;pointer-events:none;height:env(safe-area-inset-bottom,0px)'
  ...
  useEffect(() => {
    ...
    const onWarned = ({ reason }: { reason: string }) => {
      toast.warning(`You have received an account warning: ${reason}`, { duration: 0 })
    }
    ...
    return () => {
      socket.off('user_warned', onWarned)
    }
  ...
  useEffect(() => {
    if (!isAuthenticated) return
    const beat = () => {
      void api.post('/location/heartbeat', {})
    }
    beat()
  ...
  const [gateState, setGateState] = useState<'pending' | 'required' | 'anon-intent' | 'clear'>(
    'pending',
  )
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off('global_message', onGlobalMsg)
    }
  }, [socket, chatOpen])
  ...
  return (
    <div
      className="bg-background fixed left-0 right-0 top-0 flex flex-col overflow-hidden"
      style={{ height: 'var(--pwa-h, 100dvh)' }}
    >
      {/* Solid header — outside the map */}
      <header
        className="bg-background border-border relative z-50 flex flex-shrink-0 items-center justify-between border-b px-5"
        style={{
        ...
        <div className="flex items-center gap-2">
          {!isLoading && !isAuthenticated && (
            <>
              ...
              <Link
                href="/signup"
                className="bg-primary hidden rounded-lg px-3 py-1.5 text-sm font-medium text-white transition-opacity hover:opacity-90 sm:block"
              >
                Sign up
              </Link>
            </>
          ...
          {!isLoading && isAuthenticated && (
            <button
              ...
              aria-label="Settings"
              className={[
                'hover:bg-muted flex h-9 w-9 items-center justify-center rounded-lg transition-colors',
      ...
      <main className="bg-background min-h-0 flex-1 p-2">
        <div className="border-primary/25 relative h-full w-full overflow-hidden rounded-xl border-2 shadow-[0_0_0_1px_hsl(var(--primary)/0.08)]">
          {/* Map only mounts after age verification */}
          {showMap && children}

          {/* Placeholder shown while pending auth check or gate is active */}
          {!showMap && (
            <div className="bg-background absolute inset-0 flex items-center justify-center dark:bg-[hsl(260_30%_4%)]">
              {/* Light-mode gradient */}
              <div
              ...
              />
              {gateState === 'pending' && (
                <div className="border-primary relative z-10 h-7 w-7 animate-spin rounded-full border-2 border-t-transparent" />
              )}
            </div>
          )}
            ...
            <div className="absolute bottom-4 right-4 z-30 flex flex-col items-end gap-2">
              ...
              {isAuthenticated && (
                <button
                  ...
                  className={[
                    'flex items-center gap-1.5 rounded-full py-2 pl-3 pr-4 text-sm font-semibold shadow-lg transition-all',
                    isPremium
              ...
              {showMap && (
                <button
                  ...
                  className={[
                    'relative flex items-center gap-1.5 rounded-full py-2.5 pl-3 pr-4 text-sm font-semibold shadow-xl transition-all',
                    chatOpen
                      ? 'bg-primary/80 ring-primary/40 text-white ring-2'
                ...
                >
                  ...
                  {globalUnread > 0 && !chatOpen && (
                    <span className="bg-destructive absolute -right-1 -top-1 flex h-[18px] min-w-[18px] items-center justify-center rounded-full px-0.5 text-[9px] font-bold text-white shadow">
      ...
      <nav
        className="bg-background border-border flex flex-shrink-0 items-center justify-around border-t px-2"
        style={{
          paddingBottom: 'max(env(safe-area-inset-bottom, 0px), var(--pwa-sab, 0px))',
          minHeight: 'calc(4.5rem + max(env(safe-area-inset-bottom, 0px), var(--pwa-sab, 0px)))',
        }}
      >
        {navItems.map(({ href, label, Icon }) => {
          ...
          return (
            <Link
              ...
              onClick={() => {
                setChatOpen(false)
                if (href === '/messages') setTotalUnread(0)
              }}
            >
              <span className="relative">
                <Icon />
                {showBadge && (
                  <span className="bg-primary absolute -right-1.5 -top-1 flex h-4 min-w-[16px] items-center justify-center rounded-full px-0.5 text-[9px] font-bold text-white">
