...
export default function TrackPage() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-[#0d0a16]">
      {/* Header */}
      <div
        className={[
          'border-b px-5 py-4',
          sos ? 'border-red-700 bg-red-900/60' : 'border-white/10 bg-[#1c1730]',
        ].join(' ')}
      >
        ...
        {info && (
          <p className="mt-0.5 text-sm text-white/60">
            {info.name}&apos;s location · Updated{' '}
            {info.updatedAt ? new Date(info.updatedAt).toLocaleTimeString() : 'never'}
          </p>
        )}
      </div>
      ...
      {info && (
        <div
          className={[
            'space-y-1 border-t px-5 py-4',
            sos ? 'border-red-700 bg-red-900/60' : 'border-white/10 bg-[#1c1730]',
          ].join(' ')}
        >
          {sos && (
            <p className="text-sm font-semibold text-red-300">
              Emergency SOS has been triggered. Contact them immediately or call 911.
            </p>
          )}
          {ended && !sos && (
            <p className="text-sm font-medium text-green-400">
              Session ended — {info.name} checked in safely.
            </p>
          )}
          {!sos && !ended && (
            <>
              <p className="text-sm text-white/80">
                Check-in expected by {new Date(info.checkinAt).toLocaleTimeString()}
              </p>
              <p className="text-xs text-white/40">
                Location is approximate (fuzzed for privacy). Updates every 30s.
              </p>
            </>
          )}
        </div>
