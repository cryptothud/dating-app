...
export const eventsApi = {
  ...
  create: (payload: CreateEventPayload): Promise<EventSummary> => api.post('/events', payload),

  rsvp: (id: string): Promise<{ conversationId: string }> => api.post(`/events/${id}/rsvp`, {}),

  cancelRsvp: (id: string): Promise<void> => api.del(`/events/${id}/rsvp`),

  deleteEvent: (id: string): Promise<void> => api.del(`/events/${id}`),
}
